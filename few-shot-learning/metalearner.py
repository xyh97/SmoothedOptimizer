...
class MetaLearner(nn.Module):
    ...
    def forward(self, inputs, hs=None, eps=0, perturb=False):
        """Args:
        ...
        if perturb:
            state_adv = grad.detach() + 0.01 * torch.randn(grad.shape).cuda().detach()
            step_size = eps / 5

            for _ in range(10):
                state_adv.requires_grad_()
                with torch.enable_grad():
                    perturbed_update, _ = self.metalstm([lstmhx, state_adv], hs[1])
                    update, _ = self.metalstm([lstmhx, grad], hs[1])
                    distance = F.mse_loss(perturbed_update, update)
                    gradient = torch.autograd.grad(distance, [state_adv])[0]
                    state_adv = state_adv.detach() + step_size * torch.sign(gradient.detach())
                    state_adv = torch.min(torch.max(state_adv, grad - eps), grad + eps)
            perturbed_inputs = state_adv.detach()
            perturbed_update, _ = self.metalstm([lstmhx, perturbed_inputs], hs[1])
            update, _ = self.metalstm([lstmhx, grad], hs[1])
            distance = F.mse_loss(update, perturbed_update)
            return update.squeeze(), distance
        else:
