...
class GOATLogger:
    ...
    def __init__(self, args):
        if args.perturb:
            if args.n_shot == 1:
                args.save = './mini-1/' + args.save + '-{}'.format(args.seed) + \
                '-perturb-' + 'eps-{}-lamb-{}'.format(args.eps, args.lamb)
            else:
                args.save = './mini-5/' + args.save + '-{}'.format(args.seed) + \
                            '-perturb-' + 'eps-{}-lamb-{}'.format(args.eps, args.lamb)
        else:
            if args.n_shot == 1:
                args.save = './mini-1/' + args.save + '-{}'.format(args.seed)
            ...
            # args.save = './save/' + args.save + '-{}'.format(args.seed)
